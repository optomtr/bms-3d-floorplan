...
//
// Почему не берём это из furniture/names.ts и furniture/library.ts: оба тянут
// за собой three.js (модели строятся геометрией). Движку вида сверху нужны два
// числа и слово — поднимать ради них 3D-библиотеку значит привязать плоский
// редактор к WebGL, которого у него нет.
//
// Таблица заведомо неполная и это нормально: незнакомая модель рисуется
// квадратом 60×60 см со своим ключом. Палитра живёт в оболочке, она и знает
// полный список.
// ---------------------------------------------------------------------------
