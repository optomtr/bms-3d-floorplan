...
import { mat, box, cyl, tint, ledFrame, cabinetUnit, METAL, WHITE, DARK, type FurnitureBuilder } from '../primitives';
...
const chandelier: FurnitureBuilder = (c) => {
...
const crystal_chandelier: FurnitureBuilder = (c) => {
  const g = new THREE.Group();
...
export const lightingModels = {
  ...
  crystal_chandelier_double: (c, opts) => {
    ...
    for (let i = 0; i < count; i++) {
      const u = crystal_chandelier(c, opts);
      u.position.x = (i - (count - 1) / 2) * gap;
      g.add(u);
    }
    return g;
