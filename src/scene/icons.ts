...
/**
 * ЗНАКИ ДЛЯ КРУЖКОВ КЛИМАТА — отдельные от ICON_PATHS.
 *
 * На общем виде этажа кружок занимает около десяти пикселей. Обычный значок с
 * тонкой обводкой там превращается в грязное пятно: три волны «тёплого пола» и
 * завитки вентилятора на восьми пикселях неотличимы друг от друга, а текстура
 * к тому же приходит с мип-уровня (72 px сжимаются до ~9), где тонкая линия
 * просто усредняется с фоном.
 *
 * Поэтому у кружка не рисунок, а СИЛУЭТ: заливка или толстая линия, крупно и
 * без мелочей. `fill` кладётся заливкой, `stroke` — линией шириной `width` в
 * тех же 24 единицах, что и сами пути. Значка нет в таблице — рисуется обычный
 * путь из ICON_PATHS, как раньше.
 */
const BADGE_GLYPHS: Record<string, { fill?: string[]; stroke?: string[]; width?: number }> = {
  // Снежинка: шесть лучей из центра. Самая узнаваемая фигура на 8 пикселях.
  snow: { stroke: ['M12 3v18', 'M4.2 7.5l15.6 9', 'M19.8 7.5l-15.6 9'], width: 3.6 },
  // Тёплый пол: толстая линия пола и одна «галочка» тепла над ней. Три волны
  // прежнего знака на этом размере сливались в кляксу.
  floorHeat: { stroke: ['M4.6 18.4h14.8', 'M5.2 12.8L12 8.8l6.8 4'], width: 3.6 },
  // Конвектор/радиатор: СПЛОШНОЕ пламя. Контурное на этом размере исчезает.
  heat: { fill: ['M12 3.5c3 3.7 5 6 5 9a5 5 0 0 1-10 0c0-1.6.7-3 1.7-4.1.4 1 1.2 1.7 2 1.8-1.2-2.1.3-5 1.3-6.7z'] },
  // Вентиляция: трёхлопастная крыльчатка — три толстых луча и втулка.
  fan: {
    stroke: ['M12 12V3.6', 'M12 12l7.3 4.2', 'M12 12L4.7 16.2'],
    width: 3.6,
    fill: ['M12 12m-2.2 0a2.2 2.2 0 1 0 4.4 0a2.2 2.2 0 1 0-4.4 0'],
  },
  // Общий «Климат»: столбик термометра с залитым шариком.
  thermo: {
    stroke: ['M12 4.6v9'],
    width: 3.6,
    fill: ['M12 16.8m-4.2 0a4.2 4.2 0 1 0 8.4 0a4.2 4.2 0 1 0-8.4 0'],
  },
};

/**
 * Кружок-значок состояния климата, для маленького спрайта РЯДОМ с «домиком»
 * комнаты. Знак берётся из BADGE_GLYPHS (силуэт, читаемый на десяти пикселях),
 * а чего там нет — из общего ICON_PATHS. Рисуется вектором, а не шрифтом:
...
export function drawBadgeCanvas(iconName: string, filled: boolean): HTMLCanvasElement {
  ...
  const glyph = BADGE_GLYPHS[iconName];
  const S = glyph ? 46 : 40;
  const k = S / 24;
  ctx.save();
  ctx.translate((SZ - S) / 2, (SZ - S) / 2);
  ctx.scale(k, k);
  const ink = filled ? '#14171c' : '#ffffff';
  ctx.strokeStyle = ink;
  ctx.fillStyle = ink;
  ctx.lineJoin = 'round';
  ctx.lineCap = 'round';
  for (const d of glyph?.fill ?? []) ctx.fill(new Path2D(d));
  ctx.lineWidth = glyph?.width ?? 2.6;
  for (const d of glyph?.stroke ?? ICON_PATHS[iconName] ?? ICON_PATHS.dot) ctx.stroke(new Path2D(d));
  ctx.restore();
