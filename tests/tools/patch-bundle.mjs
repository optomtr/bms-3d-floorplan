...
export const PATCHES = {
  ...
  // --- 31. Потеря контекста WebGL ------------------------------------------
  'break-no-gl-recovery': {
    kind: 'break',
    note: 'сцена больше не пересобирается после webglcontextlost (как было до 0.180.0)',
    edits: [
      {
        // Якорь — вся установка обработчиков разом: если вдруг соберётся
        // другой код, правка не найдётся и инструмент упадёт, а не соврёт.
        find:
          '  e.addEventListener(\n    "webglcontextlost",\n    (i) => {\n      i.preventDefault(), setTimeout(n, 0);\n    },\n    !1\n  ), e.addEventListener("webglcontextrestored", () => setTimeout(n, 0), !1);',
        replace: '  void n;',
      },
    ],
  },
